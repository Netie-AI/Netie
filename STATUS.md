...
- Crew portable contract: wrap (empty wrap / extra unwrapped names refuse), HITL on known writes, cap-2 hard refuse above 2, parent-run, `skill` kind refused until a registry row exists, factory `index()` drops prompts, ticket runner (refusal on the board, ticket stays open), session view (no transcript), checkpoint/summarise ids-only, ledger refuses skill_body on append, seat router names billing-bypass products, WIP-2, OV gate.
