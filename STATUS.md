...
- Product specs: DMS Space ACL (warehouse bind, SQL required, row copy, SQL cannot name an ungranted table), AirGPT retrieve stays in one Space (incomplete/unlabeled not cited), Pointer UACC wrap (68 names behind Cortex; planner/history/clipboard/JS/uncropped screenshot/process-list/secret-hotkey refuse), Space preview/write refuses secrets / ungated OCR, Control board + session (no prompts / no transcript).
