...
- Sibling push 403: patches in `docs/patches/`. Constructor compiler **16 passed**. OpenVault: 15 sorts + 4 execution shapes. `/v1` last-hop usage is measured; context-relay persists caller handoff per issued seat; Anthropic-only shape hops name the skip (no Messages API). Tests **72 passed**. C2/MIN_TESTS stand. No Cortex #42.
