...
- Sibling push 403: patches in `docs/patches/`. Constructor compiler **6 passed**. OpenVault: 15 sorts + 4 execution shapes (`fusion`/`pipeline`/`context-relay`/`auto` are not sorts). Strategy+shape tests **35 passed**. C2/MIN_TESTS stand. No Cortex #42.
