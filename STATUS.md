...
- Crew portable contract: wrap, cap-2 hard refuse above 2, parent-run, factory WIP-2, OV gate, seat router.
...
- Sibling push 403: patches in `docs/patches/`. Constructor compiler **16 passed**. OpenVault: 15 sorts + 4 execution shapes. `/v1` last-hop usage is measured; context-relay persists caller handoff per issued seat; Anthropic-only shape hops name the skip (no Messages API); no matching hop is 503 not empty. Tests **73 passed**. C2/MIN_TESTS stand. No Cortex #42.
