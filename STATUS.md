...
- Product specs: DMS Space ACL (warehouse bind, SQL required, row copy), AirGPT retrieve stays in one Space (incomplete/unlabeled not cited), Pointer unlabeled / secret-field click, Space leave-machine (`user.env` refused), Control board + session (no prompts / no transcript).
