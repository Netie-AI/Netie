...
- Product specs: DMS Space ACL (warehouse bind, SQL required, row copy, SQL cannot name an ungranted table, bronze browse still needs grant), AirGPT retrieve stays in one Space (incomplete/unlabeled/`chat_*.md` not cited; file mention stays in Space), Pointer UACC wrap (68 names behind Cortex; planner/history/clipboard/JS/uncropped screenshot/page-dump/process-list/secret-hotkey refuse), Space preview/write refuses secrets / ungated OCR / ungated chat-over-preview / over-budget excerpts, Control board + session (no prompts / no transcript).
