...
- Sibling push 403: patches in `docs/patches/`. Constructor compiler **14 passed**. OpenVault: 15 sorts + 4 execution shapes. `/v1` last-hop usage is measured; context-relay persists caller handoff in-process; Anthropic-only shape hops name the skip (no Messages API). Tests **70 passed**. C2/MIN_TESTS stand. No Cortex #42.
