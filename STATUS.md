...
- Sibling push 403: patches in `docs/patches/`. Constructor compiler **62 passed** (26 patches; Cortex posts compiled IR in Kahn order, not canvas array order; `engine.js` loads before `app.js`; picking `object_type` does not invent `sku`; notes stay off IR). OpenVault: 15 sorts + 4 execution shapes. `/v1` last-hop usage is measured; context-relay persists caller handoff per issued seat (32-blob cap); Anthropic-only shape hops name the skip (no Messages API); no matching hop is 503 not empty; execution-shape serves() is catalog-true; `parallel` / quota-fetch / sqlite persist / autoCombo / compress / MCP/A2A / `quota-share` is 501; hop posts drop `combo` / `skill_body`; sidecar `metadata` / `extra` crew keys are 400; `/v1` Crew body is 400 `openvault_crew_body`. Live deploys call `claim_deploy` (`from netie.route import report_deploy` when installed). `/api/crew/gate` calls `check_crew_gate` (`from netie.crew import refuse_crew_gate` when installed). Tests **145 passed** routing plus **4** claim plus **4** crew-netie (28 product patches). C2/MIN_TESTS stand. No Cortex #42.
