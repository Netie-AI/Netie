...
AI/OCR leave without OpenVault `allowed=true` is denied. `user.env` / `.env` / `env.local` key writes are denied even when not marked plaintext. Local credential-store scan is denied. Peek-class preview refuses env/key files. Poor local OCR is not a Baidu leave-machine grant. `python3 scripts/test_space_leave.py`.
