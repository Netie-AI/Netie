...
| **OpenVault FreeRoute** vs OmniRoute | `diegosouzapw/OmniRoute` MIT (`2acbfc6`) | MIT | **4 / 10** as a gateway. **6 / 10** as a vault+gate | OmniRoute: **19** user-facing strategies. FreeRoute **main still 8**. This VM patches through 15th `cache-optimized` (rendezvous hash of caller cache_key; no prefix analyzer). Also 9-14: strict-random, lkgp, context-optimized, headroom, reset-window, reset-aware. `test_route_strategies.py` **22 passed**. Push 403. Missing: context-relay, auto, fusion, pipeline. OpenVault **main CI green**. | Land the patches. Keep porting algorithms. Do not vendor OmniRoute. |
...
- OpenVault clone HEAD `62bb1c7` (2026-08-27): `route/strategies.py` on **main** still 8 of 19. After routing patches through `openvault-cache-optimized.patch`: 15 of 19, `pytest tests/test_route_strategies.py` -> **22 passed**. `STATUS.md` ~78%; HT1 not done. Earlier full OpenMW `pytest tests -q` after detect-stacks unskip: 840 passed, 4 skipped (DPAPI). Sibling push 403. **GitHub `main` CI success** 2026-08-27 (docs/status #45).
