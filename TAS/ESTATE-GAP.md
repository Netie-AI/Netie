...
| **Constructor** vs React Flow | `xyflow/xyflow` (`@xyflow/react`) | MIT | **2 / 10** as a node editor. **4 / 10** as a Cortex IR compiler | Live clone: 11 files, custom canvas. This VM: `node --test tests/compiler.test.cjs` **21 passed** (empty graph / unknown kind / cycle / dangling edge / missing id / duplicate id invent no Cortex nodes; `ghostWalk` refuses those instead of walking `topo()` leftovers; connector-only does not invent `EMIT`; unlabeled `tool_call` does not invent `export_pptx`; unlabeled object does not invent `inventory`; inspect shows `(pick)` not a silent first-choice). Default branch **pages.yml green**; **no unit-test workflow** on HEAD. Push 403. See `TAS/TAS-CONSTRUCTOR.md`. | Land the test branch when write access exists. `npm i @xyflow/react` only if the canvas must feel like React Flow. |
...
- Constructor clone: `node --test tests/compiler.test.cjs` -> 21 passed after constructor-compiler-tests, empty-graph, ir-refuse, ir-ids, ghost-refuse, ir-emit, tool-action, inspect-action, and inspect-object patches. Sibling push 403. Default **pages.yml** green; no `test.yml` on HEAD.
