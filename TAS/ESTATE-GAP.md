...
**This VM clones public:** Netie, Cortex (`bf4ecee`), OpenVault (`62bb1c7`), constructor (`eebff20`), Pointer, dms, netie-control, Netie-KB. **Still missing:** Cortex-Crew, Space, AirGPT. Sibling **push 403**. Secret scan of those HEADs: CLEAN (no live `sk-or-v1-` / `csk-` dumps).
...
| **Pointer** vs Perplexity Computer / UACC | `uacc` on PyPI; `e2b-dev/open-computer-use` Apache-2.0 | MIT / Apache | **UNVERIFIABLE** HEAD. Fail-closed click spec exists | Pointer repo is public. `scripts/pointer_click.py` refuses unlabeled / no Cortex intent / password-OTP-cookie fields and does not touch `os.environ`. `scripts/pointer_hands.py` MCP-wraps UACC 1.1.0's 68 names behind a Cortex allow: planner/workflow/memory/history/clipboard/JS/ungated leave-machine refuse; uncropped or secret-field screenshot/snapshot/diff/visual-detect/`get_screen_info`/`get_screen_info_enhanced`/`vlm_locate_element` refuse; `browser_get_page_info` / `list_processes` / `get_system_info` / `list_windows` / `get_active_window` refuse; hover/drag/`wait_for_element` reuse fail-closed click; `hotkey` / `scroll` / `paint_image` / `paint_preset` on a secret field (or uncropped paint) refuse; `acknowledge_user_override` / `set_kill_distance` refuse. `bind_computer` refuses e2b / Perplexity Computer / open-computer-use (local tray, not a hosted computer). Do not import `uacc`. | Measure Pointer HEAD. Depend UACC under this wrap if the 68 tools are the gap. |
...
- Cortex / DMS / AirGPT / Pointer / Space / Control: Cortex, dms, Pointer, netie-control, Netie-KB **cloned public 2026-08-28** (secret scan CLEAN). AirGPT / Space / Cortex-Crew still missing. Portable specs: `dms_space_acl.py`, `airgpt_chunk.py`, `pointer_click.py`, `pointer_hands.py`, `constructor_ir.py`, `crew_durable.py`, `freeroute_free_pool.py`.
...
|---|---|---|---|
| `E:\\Cortex\\myactiveflow`, `myactivepieces` | **Constructor first** | 26 compiler patches, **62 passed**. Portable `scripts/constructor_honesty.py` plus `constructor_ir.py` + `constructor_action_bind.py` | Land patches on constructor. Unknown piece refuses. No n8n clone. |
| `E:\\Netie\\mygastown`, `E:\\openworker`, `E:\\mydeepagents` | **Cortex-Crew** | `scripts/crew_*.py` wrap/HITL/cap-2/OV gate/ids-only checkpoint. **New:** `scripts/crew_durable.py` resumes after process death from ids, prompts from tickets | Create `Netie-AI/Cortex-Crew`. `uv add deepagents`. Import Netie wrap. Study OpenWork session UX. Do not vendor `ee/` or gastown. |
...
| `E:\\Netie\\myOmniRoute` | **OpenVault FreeRoute** | 15 sorts + 4 shapes in patches (push 403). **New:** `scripts/freeroute_free_pool.py` (free tier only; empty pool 503 + register_url; no quota fetch; no invented keys) | Land patches + `openvault-crew-gate.patch`. Wire `assist_free_pool` onto `/v1` or providers/free. Do not vendor OmniRoute. |
| `E:\\mycogitorium`, `E:\\mysemantica`, `E:\\myzep` | DMS / Palantir-class | Portable `dms_space_acl.py`. dms repo now public | PRD-001 Space ACL callers. Ontology/evidence from those trees as study notes. Do not clone Palantir. |
...
| `E:\\Cortex\\Windows-MCP` | Pointer | `pointer_click.py` + `pointer_hands.py` (68 UACC names). Pointer repo now public | Measure HEAD. Depend Windows-MCP/UACC under the wrap. |
...
1. **Constructor** - import `constructor_ir.compile_ir` / `bind_action`; rebase `app.js` inspect patches; add `test.yml` on constructor.
2. **Cortex-Crew** - new empty repo; import all `scripts/crew_*.py` including `crew_durable.py`; `uv add deepagents`; fold Control board; WIP 2.
3. **OpenVault** - apply `docs/patches/` in README order; add free-pool assist; never commit provider keys.
4. **DMS Palantir-next** - wire `dms_space_acl.py`; no second warehouse brain.
5. **Pointer** - measure public Pointer vs `pointer_hands.py`; Windows-MCP behind Cortex only.
