...
Executable contract in this repo (Cortex remote still 404): `scripts/cortex_path.py`. `jepa` / `gen-cfsm` candidates are refused. DMS answers are tagged `keyword_cascade`; C7 SQL is `off`. Ungoverned writes other than `export_pptx` are refused. A tool that skips `tool_runner` is refused. `python3 scripts/test_cortex_path.py`.
