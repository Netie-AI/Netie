...
- Constructor clone: `node --test tests/compiler.test.cjs` -> 28 passed after constructor-compiler-tests, empty-graph, ir-refuse, ir-ids, ghost-refuse, ir-emit, tool-action, inspect-action, inspect-object, inspect-tier, chat-object, topo-leftover, ir-entry, and ir-output patches. Sibling push 403. Default **pages.yml** green; no `test.yml` on HEAD.
