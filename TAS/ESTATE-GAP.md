...
| **Cortex-Crew** vs Deep Agents | `langchain-ai/deepagents` 0.7.9 | MIT | **0 / 10** as a product. **2 / 10** as a portable wrap | No `Netie-AI/Cortex-Crew` remote. Netie `scripts/crew_*.py` now: wrap (empty wrap and extra unwrapped names refuse), HITL on known writes, cap-2 hard refuse above 2, parent-run graph, `skill` kind refused until a registry row exists, ticket runner (Cortex refusal on the board, ticket stays open), verify, budget, ledger, OV gate. Deep Agents `create_deep_agent(tools=require_wrapped(...))` is the depend path. Talon README: not a production security boundary. OpenVault `POST /api/crew/gate` still not on main; focused patch `openvault-crew-gate.patch` fail-closes unknown kinds (incl. `skill`) and strips bodies. | Create repo. `uv add deepagents`. Wrap every tool. Land the gate patch. |
...
| Crew repo + Deep Agents wrap | Crew | `PRD-002` drafted; queued after Space boundary. Portable HITL + ticket runner exist in Netie (`scripts/crew_runner.py`). OpenVault #44 crew_gate exists; focused `openvault-crew-gate.patch` fail-closes unknown kinds on `main`. |
