...
| **Cortex-Crew** vs Deep Agents | `langchain-ai/deepagents` 0.7.9 | MIT | **0 / 10** as a product. **2 / 10** as a portable wrap | No `Netie-AI/Cortex-Crew` remote. Netie `scripts/crew_*.py` now: wrap, cap-2, parent-run graph, verify, budget, ledger, OV gate. Deep Agents `create_deep_agent(tools=[...])` is the depend path. Talon README: not a production security boundary. OpenVault `POST /api/crew/gate` still not on main; focused patch `openvault-crew-gate.patch` fail-closes unknown kinds (incl. `skill`) and strips bodies. | Create repo. `uv add deepagents`. Wrap every tool. Land the gate patch. |
