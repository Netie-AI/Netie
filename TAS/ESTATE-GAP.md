...
| **OpenVault FreeRoute** vs OmniRoute | `diegosouzapw/OmniRoute` MIT (`2acbfc6`) | MIT | **4 / 10** as a gateway. **6 / 10** as a vault+gate | OmniRoute: **19** user-facing strategies. FreeRoute **main still 8**. This VM patches: `strict-random` (9th, deck + remainder shuffle, OmniRoute #3959) then `lkgp` (10th, last successful `execution_key` first; fail of that key clears the pin; not OmniRoute SQLite provider+connection). `test_route_strategies.py` **12 passed**. Push 403. Missing: context-relay, reset-aware, reset-window, headroom, auto, context-optimized, cache-optimized, fusion, pipeline. OpenVault **main CI green**. | Land the patches. Keep porting algorithms. Do not vendor OmniRoute. |
...
- OpenVault clone HEAD `62bb1c7` (2026-08-27): `route/strategies.py` on **main** still 8 of 19. After `docs/patches/openvault-strict-random.patch` + `openvault-lkgp.patch`: 10 of 19, `pytest tests/test_route_strategies.py` -> **12 passed**. `STATUS.md` ~78%; HT1 not done. Earlier full OpenMW `pytest tests -q` after detect-stacks unskip: 840 passed, 4 skipped (DPAPI). Sibling push 403. **GitHub `main` CI success** 2026-08-27 (docs/status #45).
