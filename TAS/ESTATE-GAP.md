...
| **Cortex-Crew** vs Deep Agents | `langchain-ai/deepagents` 0.7.9 | MIT | **0 / 10** as a product. **2 / 10** as a portable wrap | No `Netie-AI/Cortex-Crew` remote. Netie `scripts/crew_*.py` now: wrap (empty wrap and extra unwrapped names refuse; Deep Agents builtins `ls`/`read_file`/`write_file`/`edit_file`/`execute`/`task`/`write_todos` refuse even if a gate would allow), HITL on known writes, cap-2 hard refuse above 2, parent-run graph, `skill` kind refused until a registry row exists, ticket runner (Cortex refusal on the board, ticket stays open), checkpoint/summarise ids-only (resume cannot recover a prompt), verify, budget, ledger, OV gate. Deep Agents `create_deep_agent(tools=require_wrapped(...))` is the depend path. Talon README: not a production security boundary. OpenVault `POST /api/crew/gate` still not on main; focused patch `openvault-crew-gate.patch` fail-closes unknown kinds (incl. `skill`) and strips bodies. | Create repo. `uv add deepagents`. Wrap every tool. Land the gate patch. |
...
Netie portable contract (this repo, 2026-08-27): wrap (empty wrap and extra unwrapped names refuse; Deep Agents builtins `ls`/`read_file`/`write_file`/`edit_file`/`execute`/`task`/`write_todos` refuse even if a gate would allow), cap-2 parallel (hard refuse above 2), parent-run graph (`crew_runs.py`: children cannot replace parent, WIP 2, named deficit, OV gate; `skill` kind refused until a registry row exists), factory `index()` drops prompts, ledger refuses skill_body on append, `close_ticket` refuses same-run verify, token budget (HITL / builtin / skill_body refusals do not spend), hash ledger, OV gate strips `skill_body`, checkpoint/summarise ids-only (`scripts/crew_checkpoint.py`; resume cannot recover a prompt).
