...
| **OpenVault FreeRoute** vs OmniRoute | `diegosouzapw/OmniRoute` MIT (`2acbfc6`) | MIT | **4 / 10** as a gateway. **6 / 10** as a vault+gate | OmniRoute: **19** user-facing strategies. FreeRoute **main still 8**. This VM patches through 13th `reset-window` (soonest remaining ms; no quota fetch; no tie-band rotate). Also 9-12: strict-random, lkgp, context-optimized, headroom. `test_route_strategies.py` **18 passed**. Push 403. Missing: context-relay, reset-aware, auto, cache-optimized, fusion, pipeline. OpenVault **main CI green**. | Land the patches. Keep porting algorithms. Do not vendor OmniRoute. |
...
- OpenVault clone HEAD `62bb1c7` (2026-08-27): `route/strategies.py` on **main** still 8 of 19. After routing patches through `openvault-reset-window.patch`: 13 of 19, `pytest tests/test_route_strategies.py` -> **18 passed**. `STATUS.md` ~78%; HT1 not done. Earlier full OpenMW `pytest tests -q` after detect-stacks unskip: 840 passed, 4 skipped (DPAPI). Sibling push 403. **GitHub `main` CI success** 2026-08-27 (docs/status #45).
