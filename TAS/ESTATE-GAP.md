...
| **OpenVault FreeRoute** vs OmniRoute | `diegosouzapw/OmniRoute` MIT (`2acbfc6`) | MIT | **4 / 10** as a gateway. **6 / 10** as a vault+gate | OmniRoute: **19** user-facing strategies. FreeRoute **main still 8**. This VM: `strict-random` (deck + remainder shuffle, OmniRoute #3959 algorithm port) measured 9/9 strategy tests on an OpenVault worktree. Patch `docs/patches/openvault-strict-random.patch` (push 403). Missing: context-relay, reset-aware, reset-window, headroom, auto, lkgp, context-optimized, cache-optimized, fusion, pipeline. OpenVault **main CI green**. | Land the patch. Keep porting algorithms. Do not vendor OmniRoute. |
