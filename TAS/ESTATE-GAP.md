...
| **OpenVault FreeRoute** vs OmniRoute | `diegosouzapw/OmniRoute` MIT (`2acbfc6`) | MIT | **4 / 10** as a gateway. **6 / 10** as a vault+gate | OmniRoute: **19** user-facing names. FreeRoute **main still 8**. This VM: **15 sorts** (through `cache-optimized`) plus **4 execution shapes** (`fusion` panel+judge, `pipeline` sequential, `context-relay` first-available + warning-band handoff, `auto` must resolve to a sort). `apply_strategy` refuses the 4. Chat `/v1` is not wired. No autoCombo engine, no Codex quota fetch, no quorum-grace. Strategy+shape tests **35 passed**. Do not count 19 sorts. Push 403. OpenVault **main CI green**. | Land the patches. Wire `run_fusion` / `run_pipeline` into chat. Do not vendor OmniRoute. |
...
- OpenVault clone HEAD `62bb1c7` (2026-08-27): `route/strategies.py` on **main** still 8 of 19. After routing patches through `openvault-execution-shapes.patch`: 15 sorts + 4 named execution shapes, `pytest tests/test_route_strategies.py tests/test_execution_shapes.py` -> **35 passed**. `STATUS.md` ~78%; HT1 not done. Earlier full OpenMW `pytest tests -q` after detect-stacks unskip: 840 passed, 4 skipped (DPAPI). Sibling push 403. **GitHub `main` CI success** 2026-08-27 (docs/status #45).
