...
| TAS-AIRGPT chunker corpus (tables, repeated headers, labels, multilingual embedding choice) | AirGPT | Repo not in this environment. Portable `retrieve_space` does not cite another Space. `TAS/TAS-AIRGPT.md` is the hole list. |
| TAS-POINTER vs UACC tool-for-tool | Pointer | Repo not in this environment. Portable wrap lists UACC 1.1.0's 68 names and refuses planner/history/clipboard/JS/uncropped screenshot/process list. `TAS/TAS-POINTER.md`. |
