...
MCP wrap of UACC 1.1.0's **68** public tool names: `scripts/pointer_hands.py`. Cortex must allow the name. Planner / workflow / memory / `get_action_history` / clipboard / `browser_execute_js` / ungated `open_url` refuse. Screenshot / snapshot / screen-diff need a labeled non-secret crop (`screenshot_uncropped` otherwise). `list_processes` refuses. Click/type hands reuse the fail-closed click. `hotkey` on a password/OTP field refuses. We do not import `uacc`. `python3 scripts/test_pointer_click.py` and `python3 scripts/test_pointer_hands.py`. Not a UACC clone.
