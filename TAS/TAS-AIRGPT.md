...
Portable corpus in this repo (AirGPT still 404): `scripts/airgpt_chunk.py`. Repeated headers are not extra rows, ragged short rows do not invent cells, extra cells are dropped (not a nameless column), `# warehouse:` labels stick to following rows. `python3 scripts/test_airgpt_chunk.py`. Not LlamaIndex; a splitter we own until HEAD is measured.
