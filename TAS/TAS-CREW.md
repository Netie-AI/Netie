...
| Skill browser | Netie-KB promotions via `register_from_kb` (id + source only; no skill_body) + `skills/` directory; pick, demote, improve |
...
**Shipped:** no Crew product repo. Netie `scripts/crew_*.py` is the portable wrap/verify/budget/ledger/gate the Crew repo must import (`register_from_kb` is ids-only), not a runtime to pretend is Crew.
