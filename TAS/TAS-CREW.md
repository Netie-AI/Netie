...
| Need | Closest | Action |
|---|---|---|
| Harness (subagents, summarise, skills, HITL, checkpoints) | `langchain-ai/deepagents` 0.7.9 (MIT, on LangGraph) | **Depend.** Wrap every tool with Cortex gates. Their default is "trust the LLM"; Talon README says it is not a production security boundary. Ours is the opposite. `bind_deep_agent` -> `create_deep_agent(tools=require_wrapped(...))` with a `HarnessProfile` that excludes filesystem / `execute` / `task` / `write_todos` and disables the general-purpose subagent. Extra factory knobs (`system_prompt` / `middleware` / `backend` / `permissions` / `skills` / `response_format` / `debug`) refuse. Empty wrap refuses. Extra unwrapped names refuse. Known writes need `operator_confirm=True`. Deep Agents builtins `ls` / `read_file` / `write_file` / `edit_file` / `delete` / `glob` / `grep` / `execute` / `task` / `write_todos` refuse even if a Cortex gate would allow (`task` is ungoverned fan-out; use `crew_parallel` cap-2). Checkpoints and summarise are ids/counts only (`scripts/crew_checkpoint.py`); they do not persist a transcript. |
