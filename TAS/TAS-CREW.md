...
**First artefact (in Netie, until the Crew repo exists):** public import `from netie.crew import bind_deep_agent, crew_harness_profile, TokenBudget` (`uv add git+https://github.com/Netie-AI/Netie.git`). Wrap (empty wrap and extra unwrapped names refuse; Deep Agents builtins `ls` / `read_file` / `write_file` / `edit_file` / `delete` / `glob` / `grep` / `execute` / `task` / `write_todos` refuse even if a gate would allow). MIT `create_deep_agent` only after wrap; `crew_harness_profile` excludes those builtins, drops `SummarizationMiddleware` (else `/conversation_history/{session}.md` is a transcript dump), and disables the general-purpose `task` subagent; injected `factory=` without `register=` refuses (else builtins stay on); `bind_deep_agent` / `execute_capability` / `execute_capabilities` / `run_open_ticket` / `run_batch` require `TokenBudget` (else unbounded spend); wrap `skill_body` refuses and does not spend; over-budget wrap / ticket does not execute (ticket stays open); `skills` / `memory` / `subagents` / `system_prompt` / `middleware` / `backend` / `response_format` / `debug` / transcript checkpointer refuse. HITL on known writes (`export_pptx` / `item.intake` / `amend.apply` / `call_action` need `operator_confirm=True`), OpenWork-shaped `search_capabilities` / `execute_capability` (ungranted / builtin / billing-bypass / skill_body refuse; leave-machine needs OV; `execute_capability(ov=)` POSTs skill ids; session permissions drop builtins; `execute_capabilities` is cap-2), cap-2 parallel (hard refuse above 2), verify, budget, ledger (refuses skill_body on append), OV gate (`skill` kind refused until a registry row exists; `register_from_kb` / `register_index` mint that row from a Netie-KB index id, never a body), parent-run graph (default child kind is `service`, not `skill`), factory (PRD slice refuses without out-of-scope; WIP 2 epics; close updates the epic task list; `index()` drops prompts), ticket runner (Cortex refusal on the board, ticket stays open, prompt never goes to a child; leave-machine names POST skill ids via `run_open_ticket(ov=)` / `run_batch(ov=)` / `wrap_deepagents_tools(ov=)` / `execute_capability`; Cortex tools stay on `prepare_tool` and never hit `refuse_crew_gate`; `board_from_runs` stitches Factory + graph runs + skill ids via `board_index`), session view (ids only, skill ids, no transcript), checkpoint/summarise (ids and counts only; resume cannot recover a prompt; skill ids + source persist; persist/resume/`checkpoint_graph` default to `graph.ov.registry`; summarise counts skill ids, not bodies), `seat_router.py`. Child jobs that still carry `skill_body` / transcript, HITL writes without `operator_confirm`, or Deep Agents builtins are refused and do not spend budget. Tools Cortex or OpenVault refuse do not run. This is not Deep Agents and not a second `dag_runner`.
...
| Surface | Role |
|---|---|
| Crew board (absorb Control) | Factory tickets/epics + run/ledger/refusal/skill-id cards via `board_from_runs`; no prompts; no dag_runner |
| Session view | one live run: ids, todos, permissions, hand-off id, skill ids; no transcript |
| Skill browser | Netie-KB promotions via `register_from_kb` / `register_index` (id + source only; no skill_body) + `skills/` directory; pick, demote, improve |
...
| Need | Closest | Action |
|---|---|---|
| Harness (subagents, summarise, skills, HITL, checkpoints) | `langchain-ai/deepagents` 0.7.9 (MIT, on LangGraph) | **Depend.** Wrap every tool with Cortex gates. Their default is "trust the LLM"; Talon README says it is not a production security boundary. Ours is the opposite. `bind_deep_agent` -> `create_deep_agent(tools=require_wrapped(...))` with `crew_harness_profile` that excludes filesystem / `execute` / `task` / `write_todos`, drops `SummarizationMiddleware` (else `/conversation_history/{session}.md` is a transcript dump), and disables the general-purpose subagent. `bind_deep_agent` requires `TokenBudget`. Wrap `skill_body` refuses and does not spend; over-budget wrap does not execute. Injected `factory=` without `register=` refuses. Extra factory knobs (`system_prompt` / `middleware` / `backend` / `permissions` / `skills` / `response_format` / `debug`) refuse. Empty wrap refuses. Extra unwrapped names refuse. Known writes need `operator_confirm=True`. Deep Agents builtins `ls` / `read_file` / `write_file` / `edit_file` / `delete` / `glob` / `grep` / `execute` / `task` / `write_todos` refuse even if a Cortex gate would allow (`task` is ungoverned fan-out; use `crew_parallel` cap-2). Checkpoints and summarise are ids/counts only (`scripts/crew_checkpoint.py`); they do not persist a transcript. Summarise counts skill ids. `checkpoint_graph` defaults to `graph.ov.registry`. |
...
**Shipped:** no Crew product repo. Netie `scripts/crew_*.py` is the portable wrap/verify/budget/ledger/gate the Crew repo must import (`register_from_kb` / `register_index` are ids-only), not a runtime to pretend is Crew.
