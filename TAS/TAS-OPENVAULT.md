...
Portable FREE-token assist (this repo): `scripts/freeroute_free_pool.py`. Catalog rows with `tier` in `{free, freemium, local}` are the pool. Empty pool is 503 plus `register_url` / `free_notes` help, not a silent paid walk (`allow_paid` is opt-in). Catalog must not carry `api_key`. Quota fetch / autoCombo / parallel stay 501. `E:\\Netie\\myOmniRoute` is the analogue to port, not a tree to copy.
