...
This VM patches (not on main, push 403): 15 *sorts* through `cache-optimized`, then 4 *execution shapes* that `apply_strategy` refuses. Portable: `scripts/freeroute_execution.py` (`dispatch_combo`, `hop_call_model`, `classify_hop_status`, `sse_wrap_text`, `relay_available_from_body`). `/v1` fail-closes nameless fusion; with `combo.models` sequential hop posts classify like the key walk; last hop may SSE; context-relay uses caller available/handoff; usage names the last hop (not OmniRoute parallel quorum-grace, no Codex fetch). `model: auto` stays catalog pick.
...
**Shipped enough to test:** vault, gate refuse paths, FreeRoute metering/tests, 8 sorts on `main` (9th-15th sorts plus 4 execution-shape contracts in `docs/patches/`, 64 tests passed here, push 403), Pages adapter code, Next UI, Electron shell, NVMe library, CI.
