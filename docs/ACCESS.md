...
Measured on this VM 2026-08-28 against public clones: Netie unittest **295 passed**; constructor (18 patches) `node --test` **42 passed**; OpenVault OpenMW (25 patches) routing+chat **90 passed**; prior full OpenMW suite on 23 patches was **912 passed, 4 skipped**. Re-probed same day: Cortex/Pointer/Space/KB/Crew/AirGPT still 404.
...
2. Org app access: `https://github.com/organizations/Netie-AI/settings/installations` -> **Cursor** -> Repository access -> **select**:
   ...
   - `Netie-AI/Netie-KB`
3. Create empty `Netie-AI/Cortex-Crew` (do not clone OpenWork or Grok Bot). Then add that repo to the same App list. `uv add --editable git+https://github.com/Netie-AI/Netie.git` then `from netie.crew import bind_deep_agent`. `uv add deepagents`.
