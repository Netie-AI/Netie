...
Measured on this VM 2026-08-28 against public clones: constructor `4896ddd` (13 patches) `node --test` **31 passed**; OpenVault OpenMW routing+chat+crew-gate+free-pool+`/api/route/free` pytest via sibling gate. Prior full OpenMW suite on 23 patches was **912 passed, 4 skipped**. Cortex/Pointer/dms/control/KB clone. AirGPT / Space / Cortex-Crew still 404.
...
`cursor[bot]` cannot push to OpenVault or constructor. On a machine that can, apply patches in the order listed in `docs/patches/README.md`. Constructor public HEAD is `4896ddd`: stop after `constructor-inspect-4896ddd.patch` (31 passed). Do not apply original `inspect-object` onward on that SHA. OpenVault: do not skip hop-walk through `openvault-free-pool-route.patch`. Later eebff20 patches assume inspect-object. dms: `dms-demo-acl-resolve.patch` makes `demo_acl` return `resolve_session_acl`.
