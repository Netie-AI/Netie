...
| P-002 | Windows type/hotkey/screenshot proven on hardware | Run `python -m pointer live-click` on the founder Windows laptop (shots under `.pointer-state/shots`, not `/tmp`) and attach the JSON plus a screenshot file |
...
| P-007 | $1M authorized bounty hunting | HackerOne/Bugcrowd account in-scope programs only; no unauthorised access; payouts_enabled on Stripe |
