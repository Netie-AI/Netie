...
def laptop_next_steps() -> list[str]:
    return [
        "On the Windows laptop, not the cloud VM: copy Pointer/ then double-click scripts\\install_windows.cmd (or powershell -File scripts/install_windows.ps1)",
...
def write_next(state_dir: Path) -> Path:
    ...
    lines = [
        ...
        *[f"{i}. {step}" for i, step in enumerate(laptop_next_steps(), start=1)],
        ...
        "Cash paths (optional): powershell -File scripts/open_revenue.ps1 -- AI Studio, Agentic, Cinema, YC, Hacker101. Not a $1M claim.",
